...
def create_uninf_db(cur, conn):
    ...
    cur.execute("DROP VIEW IF EXISTS OrderSize;")
    cur.execute("""Create VIEW OrderSize AS
    ...
    for i in range(50):
        start = time.time()
        cur.executescript("""SELECT COUNT(*) as ANSWER
                        FROM (SELECT nos.order_id
                            FROM (SELECT o.order_id
                                    ...
                                    AND (c.customer_postal_code =
                                        (SELECT c.customer_postal_code FROM Customers_new c ORDER BY random() LIMIT 1))) nos
                            WHERE (SELECT COUNT(*) FROM Order_items_new i WHERE i.order_id = nos.order_id) > (SELECT AVG(size) FROM OrderSize));""")
        end = time.time()
...
def create_user_db(cur, conn):
    ...
    cur.executescript(""" DROP INDEX IF EXISTS idx_order_id;
                        ...
                        """)
    cur.executescript("""CREATE INDEX idx_order_id ON Orders(customer_id,order_id);
                        CREATE INDEX idx_customer_id ON Customers(customer_postal_code,customer_id);
